...
- `Llamex.ModelLoader.from_compact_map/1` で compact model map から最小 `Model` を
  ...
  遅延展開できる。
- compact Q4_0 の transformer layer 主要 attention / FFN weight
  (`attn_norm`、Q/K/V/O projection、`ffn_norm`、gate/up/down projection) も
  compact model map から遅延展開して `Model.layers` へ保持できる。
