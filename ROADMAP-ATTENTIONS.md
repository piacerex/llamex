...
- [x] sliding-window 専用 attention を追加する。
...
- [ ] FlashAttention、block sparse attention、paged KV cache、backend-native tensor cache、attention 専用 fused kernel を追加する。
  - どの Attention が性能面、品質面、性能あたり品質として優れているか検証する。
...
- [x] output logits / FFN との支配率を比較する。
...
- `Llamex.KVCache.append_window/5` を追加し、sliding-window モデルでは Attention の参照前に古い entry を破棄する。
- window pruning が発生した layer では backend prepared KV cache を破棄し、pruned entries から再構築する。
- List backend の `kv_head_count == 1` 経路では、共有 KV head を query head ごとに取り直さず再利用する。
- NxEXLA backend では既存の `Backend.qkv_heads/8` と `w_qkv` 結合を Gemma3 型の GQA fixture で固定する。
- `timing_summary.focus_components` で `attention`、`ffn`、`output_logits` を比較しやすくする。
- Profile 時の Attention は `qkv`、`kv_cache`、`attend`、`output_projection` の substep timing を返す。
- `backend_profile.kv_cache_entries` と `backend_profile.kv_cache_entry_counts` で、layer ごとの KV cache 保持量を確認する。

## 制約

- FlashAttention、block sparse attention、paged KV cache、backend-native tensor cache、attention 専用 fused kernel は未採用。
  現時点では AtomVM 想定の純 Elixir core と backend 差し替え境界を優先する。
- Gemma3 270m GGUF 実ファイルがこの作業環境に無いため、baseline profile と実モデル smoke は未取得。
- 長い prompt / 複数 token 生成での KV cache 保持量比較は未取得。
- Q/K/V 融合の効果測定には、token 評価時の `eval.layer_N.attention.qkv` と、
  NxEXLA prepare 時の `qkv_combine_milliseconds` を併用する。

## 検証順

1. Gemma3 270m Q5_K_M GGUF で baseline profile を取得する。
2. [x] List backend で sliding-window pruning の保持量と結果一致を固定する。
3. [x] GQA 専用経路を小さな fixture でテストし、List backend で結果一致を固定する。
4. [ ] NxEXLA backend で Q/K/V projection 融合の効果を測る。
5. [ ] KV cache 表現を差し替え、長い prompt / 複数 token 生成で比較する。
6. [x] 改善が確認できた経路だけを標準 Attention 経路へ統合する。
...
- [x] README または ROADMAP に、採用した Attention 経路と制約が反映されている。
