...
`prompt_eval_summary`. JSON benchmark runs include `prepared?`, generated token
IDs/pieces, context-window limits, prompt truncation metadata, and the sampler
settings, so the prepared route, generation budget, seed, and sampling options
used for each run can be audited. Use `prompt_eval_steps` to inspect prefill token-by-token timings, and
