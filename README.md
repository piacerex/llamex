...
- `Llamex.Backend.FPGA`: FPGA boundary, backed by the List fallback by default;
  `configure_runtime!/1` can delegate supported operations to a board runtime,
  and `capabilities/0` reports the delegated/fallback status
...
- Architecture: `llama`, `mistral`, `gemma3` text runtime with full
  attention/default RoPE
...
For GGUF models, each JSON result also includes `model_diagnostic`, so natural
smoke artifacts carry loadability, compatibility, eager F32 expansion, tensor
payload, chat usability, and tokenizer metadata issue fields alongside the
generated text.
Add `--include-japanese` to append a Japanese prompt to the default English
smoke prompt suite. Explicit `--prompt` values are left unchanged.
...
`Llamex.GGUF.Diagnostic.inspect_summary_binary/1` and
`inspect_summary_reader/1` provide the same compact diagnostic map for fixtures
...
attention/RoPE variant summaries, unsupported attention/RoPE feature metadata,
plus representative raw GGUF dimensions and normalized schema shapes for key
tensors. This is the fastest way to decide whether `--chat` is safe for a
checkpoint and whether tensor layout looks plausible.
...
still require unsupported runtime behavior. Gemma 3 q/k and post feed-forward
extra norms are part of the supported text runtime path.
`extra_norm_tensor_layers` lists the layer indexes and tensor parts detected for
that runtime path.
...
`Llamex.TensorStore.dequantize_compact_tensor/1` can currently expand compact
...
`Llamex.ModelLoader.from_compact_map/2` and `Llamex.GGUF.ModelLoader.load/2`
accept `compact_backend: true` with `tensor_format: :compact` to keep Q4_0
layer and output weights compact for the List backend generation path.
...
```text
architecture: llama
supported architectures: llama, gemma3, mistral
supported combinations: llama+whitespace/bpe+llama/gpt2+default/gpt2/llama-bpe+BF16/F16/F32/Q2_K/Q3_K/Q4_0/Q4_1/Q4_K/Q5_0/Q5_1/Q5_K/Q6_K/Q8_0/Q8_1/Q8_K; gemma3+whitespace/bpe+llama/gpt2+default/gpt2/llama-bpe+BF16/F16/F32/Q2_K/Q3_K/Q4_0/Q4_1/Q4_K/Q5_0/Q5_1/Q5_K/Q6_K/Q8_0/Q8_1/Q8_K; mistral+whitespace/bpe+llama/gpt2+default/gpt2/llama-bpe+BF16/F16/F32/Q2_K/Q3_K/Q4_0/Q4_1/Q4_K/Q5_0/Q5_1/Q5_K/Q6_K/Q8_0/Q8_1/Q8_K
